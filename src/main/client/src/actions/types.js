// Error Reducer
export const GET_ERRORS = 'GET_ERRORS';
export const CLEAR_ERRORS = 'CLEAR_ERRORS';

// Message Reducer
export const SET_MESSAGE = 'SET_MESSAGE';
export const CLOSE_MESSAGE = 'CLOSE_MESSAGE';

// Auth Reducer
export const SET_CURRENT_USER = 'SET_CURRENT_USER';
export const AUTH_LOADING = 'AUTH_LOADING';

// Developers Reducer
export const GET_DEVELOPERS = 'GET_DEVELOPERS';
export const DEVELOPERS_LOADING = 'DEVELOPERS_LOADING';

// User Reducer
export const GET_USER = 'GET_USER';
export const SET_USER_PROFILE = 'SET_USER_PROFILE';
export const SAVE_PROFILE_LOADING = 'SAVE_PROFILE_LOADING';
export const GET_USER_LOADING = 'GET_USER_LOADING';
export const UPDATE_USER_TITLES = 'UPDATE_USER_TITLES';
export const UPDATE_USER_TITLES_LOADING = 'UPDATE_USER_TITLES_LOADING';

// Title Reducer
export const GET_TITLES = 'GET_TITLES';
export const GET_TITLES_LOADING = 'GET_TITLES_LOADING';
